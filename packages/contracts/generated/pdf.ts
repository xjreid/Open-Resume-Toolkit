...
export type RenderPdfRequest = StyledExportRequest;

export const MAX_PDF_BYTES = 4194304;
export const MAX_PDF_PAGES = 5;
export const MAX_PDF_RENDER_HISTORY = 20;
export const PDF_PREVIEW_TTL_SECONDS = 600;
...
export interface PdfRegenerateRequest {
  contractVersion: typeof CONTRACT_VERSION;
  requestId: string;
  payload: { manifestId: string; style: DocumentStyle };
}
export interface PortablePdfRegenerateRequest {
  contractVersion: typeof CONTRACT_VERSION;
  requestId: string;
  payload: { archiveId: string; manifestId: string; style: DocumentStyle };
}
...
function bundleId(value: unknown): value is string {
  return (
    typeof value === "string" &&
    value.length > 0 &&
    value.length <= 128 &&
...
function isReceipt(
  value: unknown,
  historical = false,
): value is PdfRenderReceipt {
  return (
    record(value) &&
    Object.keys(value).length === 10 &&
    hash(value.documentSha256) &&
    (value.documentSchemaVersion === 1 || value.documentSchemaVersion === 2) &&
    hash(value.pdfSha256) &&
    (historical
      ? bundleId(value.rendererVersion)
      : value.rendererVersion === "typst-0.15.1/ort-1") &&
    (historical
      ? bundleId(value.templateId)
      : Object.values(DOCUMENT_STYLE_TEMPLATES).some(
          (style) => style.pdf === value.templateId,
        )) &&
    hash(value.templateSha256) &&
    (historical
      ? bundleId(value.fontBundleId)
