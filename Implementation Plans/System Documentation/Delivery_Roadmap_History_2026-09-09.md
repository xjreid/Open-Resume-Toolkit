# Historical delivery roadmap — archived 2026-09-09

This snapshot preserves earlier checkpoints and superseded testing requirements.
...
qualification were subsequently completed for the accepted M2 development scope.
This architecture note is historical; see current status below.


## Status

- Status: M0, M1 and M2 complete for macOS Apple Silicon development; M3 next
- Acceptance: [M0–M2 closure and known limitations](../../evidence/0.0.0-dev/m2-acceptance-closure.md)
- Owner: maintainers
...
`[HIGH]` identifies work with significant security, data-integrity, spending or
release-evidence consequences. It is a risk marker, not a completion status.
Apply the user's current model/delegation preferences: choose reasoning and review
depth proportional to risk, using stronger analysis when justified. The user's
authorization for adaptive routing supersedes older mandatory model-switch pauses.
...
**M2 is complete for macOS Apple Silicon development by user acceptance on
2026-09-09.** Menu Quit, Command-Q and saved-state persistence passed the final
exact-candidate account checks. Remaining manual checks are waived/unrun;
test-account local signing trust and archive-selection ambiguity remain explicit
limitations. See the [closure record](../../evidence/0.0.0-dev/m2-acceptance-closure.md)
and [next milestone guide](../Next_Milestones.md). No new hosted CI result is claimed.

<details>
<summary>Historical M2 implementation and acceptance chronology (superseded status)</summary>

### Historical status

**2026-09-09 final scope decision (supersedes the older acceptance matrix):**
The user waived the remaining manual checks and requested one focused app-menu
quit repair/retest before closing M2 for macOS Apple Silicon development. Round
three confirmed expired-review cancellation and Command-Q/reopen persistence;
menu Quit with the overlay frontmost still failed. Restore archive-selection
ambiguity remains inconclusive, and skipped checks remain unrun. Use the
[final quick handoff](../../evidence/0.0.0-dev/m2-quit-final-handoff.md). M0/M1
remain complete; M2 acceptance awaits this focused result. This scope decision
does not assert a new hosted CI pass or distribution/platform qualification.


**2026-09-09 acceptance update:** Native rounds one/two passed import, review,
revision/persistence, all-style multipage PDF/DOCX, publication/regeneration and
sampled accessibility/date checks. Stale review cancellation and idle-review quit
blocking are repaired locally; the full-app overlay-exit observation still needs
exact-candidate retest. A fresh signed candidate, native recovery-crash/APFS tests,
and the final account-session procedures are recorded in
[round-three preparation](../../evidence/0.0.0-dev/m2-round3-preparation.md).
M0/M1 remain complete. Final M2 native acceptance and CI for the new changes remain
open; the earlier implementation CI pass was user-reported for `295e3b2`.
...
### Accepted evidence limits and later qualification

The [closure record](../../evidence/0.0.0-dev/m2-acceptance-closure.md) is the
current acceptance authority. Earlier full-matrix requirements are superseded by
the user's waiver; unrun cases are not passes. Remaining local signing-trust
setup, broader accessibility/offline/failure-path coverage and clean-machine
packaging belong to future qualification when relevant, rather than reopening M2.
Later features must extend all-local-data deletion for their new stored state.
Windows AppContainer/Job, vault/filesystem/UI, Intel Mac dependencies and
clean-machine package qualification remain deferred platform-expansion work.
