...
The implementation security review must add the final Rust crate/build documentation, Windows Credential Manager documentation, Argon2id/XChaCha20-Poly1305 specifications and test vectors, and exact SQLCipher compile options before format freeze.
...
- [Codex app-server](https://developers.openai.com/codex/app-server) — `stdio` server lifecycle, initialization, thread/turn methods, events, account data, and protocol generation.
