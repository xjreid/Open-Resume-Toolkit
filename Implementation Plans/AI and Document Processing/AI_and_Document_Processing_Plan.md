...
Historical content stores its original renderer tuple. If that tuple is no
longer installed, ORT may regenerate a retained immutable structured source with
the current supported renderer only under a new receipt that exposes the
effective tuple and clearly disclaims byte-identical history. M2 does not retain
superseded master drafts or ship superseded executable renderer/font bundles
solely for replay.
