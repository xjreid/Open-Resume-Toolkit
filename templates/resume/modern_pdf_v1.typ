...
#set par(spacing: 0pt, justify: false, leading: 6.09375pt)
#let accent = rgb("1B5C69")
#show heading: it => it.body
#show heading.where(level: 1): set text(size: 23.25pt, weight: "regular")
...
#for p in json(bytes(sys.inputs.resume)) {
  if p.kind == "name" {
    align(center, block(inset: (top: 1.5pt), above: 0pt, below: 4pt, text(size: 23.25pt, tracking: .015em, weight: "bold", fill: rgb("133C49"), heading(level: 1, outlined: false, rich(p.runs)))))
  } else if p.kind == "contact" {
    align(center, block(inset: (top: 3pt, bottom: 3pt), above: 0pt, below: 9pt, rich(p.runs)))
  } else if p.kind == "section" {
    block(sticky: true, above: 18pt, below: 3.75pt, { text(size: 10.5pt, tracking: .08em, weight: "bold", fill: accent, heading(level: 2, outlined: true, upper(rich(p.runs)))); v(1.5pt); line(length: 100%, stroke: 1.5pt + rgb("276A78")) })
    ...
    block(sticky: p.sticky, inset: (top: row-inset, bottom: row-inset), above: 3pt, below: if p.entry_end { 10.5pt } else { 0pt },
      pad(left: content-inset, grid(columns: (1fr, .28fr), align: (left, right), column-gutter: 10.5pt,
        stack(dir: ttb, spacing: 6.09375pt, entry-title(p), ..(if p.subtitle_runs.len() > 0 { (rich(p.subtitle_runs),) } else { () })),
        rich(p.right_runs),
      ))
    )
