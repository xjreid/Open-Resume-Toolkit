...
#set text(font: "Libertinus Serif", size: 10.5pt, top-edge: .8em, bottom-edge: .2em, fill: rgb("171717"), lang: "en", fallback: false, ligatures: false)
#set par(spacing: 0pt, justify: false, leading: 4.095pt)
#let accent = rgb("243C5A")
#let rule = rgb("8A795F")
#show link: set text(fill: rgb("0A4F9E"))
#let content-inset = 10.5pt
#let row-inset = 2.65pt
#let literal(value) = {
...
#for p in json(bytes(sys.inputs.resume)) {
  if p.kind == "name" {
    align(center, block(above: 0pt, below: 4pt, text(size: 22.5pt, tracking: .015em, weight: "regular", fill: accent, rich(p.runs))))
  } else if p.kind == "contact" {
    align(center, block(inset: (top: 3pt, bottom: 3pt), above: 0pt, below: 9pt, rich(p.runs)))
  } else if p.kind == "section" {
    block(sticky: true, above: 15.75pt, below: 3.75pt, { text(size: 10.5pt, tracking: .035em, weight: "bold", fill: accent, rich(p.runs)); v(1.5pt); line(length: 100%, stroke: .75pt + rule) })
