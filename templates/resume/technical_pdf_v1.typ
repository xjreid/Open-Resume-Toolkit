...
#set par(spacing: 0pt, justify: false, leading: 1.8pt)
...
#for p in json(bytes(sys.inputs.resume)) {
  if p.kind == "name" {
    align(center, block(above: 0pt, below: 4pt, text(size: 23pt, weight: "bold", fill: accent, rich(p.runs))))
  } else if p.kind == "contact" {
    align(center, block(above: 0pt, below: 3pt, text(size: 9.5pt, rich(p.runs))))
  } else if p.kind == "section" {
    block(sticky: true, above: 9pt, below: 4pt, { text(size: 11pt, weight: "bold", fill: accent, rich(p.runs)); v(2pt); line(length: 100%, stroke: .5pt + accent) })
