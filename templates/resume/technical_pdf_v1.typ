...
#set text(font: "Liberation Serif", size: 10.125pt, top-edge: .8em, bottom-edge: .2em, fill: rgb("171717"), lang: "en", fallback: false, ligatures: false)
#set par(spacing: 0pt, justify: false, leading: 4.2525pt)
#show heading: it => it.body
#show heading.where(level: 1): set text(size: 22.5pt, weight: "regular")
#show heading.where(level: 2): set text(size: 11.25pt)
...
#for p in json(bytes(sys.inputs.resume)) {
  if p.kind == "name" {
    align(center, block(inset: (top: 1.5pt), above: 0pt, below: 4pt, text(size: 22.5pt, tracking: .015em, weight: "bold", fill: accent, heading(level: 1, outlined: false, rich(p.runs)))))
    ...
    block(sticky: p.sticky, inset: (top: row-inset, bottom: row-inset), above: 3pt, below: if p.entry_end { 10.5pt } else { 0pt },
      pad(left: content-inset, grid(columns: (1fr, .28fr), align: (left, right), column-gutter: 10.5pt,
        stack(dir: ttb, spacing: 4.2525pt, entry-title(p), ..(if p.subtitle_runs.len() > 0 { (rich(p.subtitle_runs),) } else { () })),
        rich(p.right_runs),
      ))
    )
